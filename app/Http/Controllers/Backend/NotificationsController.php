...
class NotificationsController extends Controller
{
    ...
    public function notificationList(Request $request)
    {
        ...
        $new_appointment_count = isset($user->unreadNotifications) ? $user->unreadNotifications->where('data.type', 'appointment_added')->count() : 0;
        
        return response()->json([
            'status' => true,
            'type' => $type,
            'data' => view("backend.$module_name.list", compact('notifications', 'new_appointment_count', 'all_unread_count', 'user'))->render(),
        ]);
    }
