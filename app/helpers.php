...
function getjoinUrl($access_token, $data)
{
    ...
    $description = "Dear {$emailData['user_name']},<br><br>" .
...
function getMediaUrls()
    {
        ...
        if (Setting::where('name', 'disc_type')->value('val') == 's3') {
            $files = Storage::disk('s3')->files('media');
            $aws_media = array_map(function ($file) {
        ...
        return array_merge($aws_media, $media);
    }
    ...
    if (!function_exists('setDefaultImage')) {
        ...
        {
            $defaultImagePath = '/default-image/horror_genre.png'; 
