    ...
    <form method="POST" id="form" action="{{ route('backend.tv-channel.store') }}" enctype="multipart/form-data">
        ...
        <div class="row">
            ...
            <div class="col-sm-6 mb-3">
                ...
                {{ html()->select('category_id', $tvcategory->pluck('name', 'id'), old('category_id'))->class('form-control select2')->id('category_id') }}
                @error('category_id')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
    ...
    <div id="video-inputs-container-parent">
        <div class="row video-inputs-container">
            <div class="col-sm-6 mb-3">
                ...
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="type" id="t_url" value="t_url" onchange="showStreamtypeSelection('t_url')" {{ old('type') == 't_url' ? 'checked' : '' }} checked>
                    <label class="form-check-label" for="t_url">URL</label>
                </div>
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="type" id="t_embedded" value="t_embedded" onchange="showStreamtypeSelection('t_embedded')" {{ old('type') == 't_embedded' ? 'checked' : '' }}>
            ...
            <!-- Select Box for URL -->
            <div class="col-sm-6 mb-3 d-none" id="type_url">
            ...
            <div class="col-sm-6 mb-3 d-none" id="type_embedded">
                {{ html()->label(__('movie.lbl_stream_type') . '<span class="text-danger">*</span>', 'stream_type')->class('form-control-label') }}
                {{ html()->select(
                        'stream_type',
                        $embedded->pluck('name', 'value')->prepend('Select Video Type', ''),
                        old('stream_type', 'Embedded'),
            ...
            <!-- URL Input Fields -->
            <div class="col-sm-6 mb-3 d-none" id="server_url">
                {{ html()->label(__('movie.server_url') . '<span class="text-danger">*</span>', 'server_url')->class('form-control-label') }}
                {{ html()->text('server_url')->placeholder(__('movie.server_url'))->class('form-control') }}
            </div>
            <div class="col-sm-6 mb-3 d-none" id="server_url1">
            ...
            <!-- Embedded Textarea -->
            <div class="col-sm-6 mb-3 d-none" id="embedded_textarea">
                {{ html()->label(__('movie.embedded') . '<span class="text-danger">*</span>', 'embedded')->class('form-control-label') }}
                {{ html()->textarea('embedded', old('embedded'))->class('form-control')->id('embedded')->placeholder(__('movie.embedded')) }}
            </div>
        </div>
    </div>
</div>
